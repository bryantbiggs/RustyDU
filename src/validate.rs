...
impl Validate {
        ...
        match client.post(&api_url).headers(headers).json(&payload).send().await {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    println!("Error: {} - {}", response.status(), response.text().await.unwrap_or_default());
                }
            }
    ...
    async fn submit_extraction_validation_request(&self, extractor_id: &str, response: &reqwest::Response) -> Option<serde_json::Value> {
        ...
        let url = format!("{}/{}/extractors/{}/validation/result/{}?api-version=1", self.base_url, self.project_id, extractor_id, operation_id);
        let client = Client::new();

        loop {
            match client.get(&url).header("Authorization", format!("Bearer {}", self.bearer_token)).send().await {
                Ok(response) => {
                    let response_data: serde_json::Value = match response.json().await {
                        Ok(data) => data,
                        Err(err) => {
                    ...
                    match response_data["status"].as_str() {
                                ...
                                match client.get(&url).header("Authorization", format!("Bearer {}", self.bearer_token)).send().await {
                                    Ok(response) => {
                                        ...
                                        match response_data["result"]["actionData"]["status"].as_str() {
                                            Some("Unassigned") => println!("Validation Document Extraction is unassigned. Waiting..."),
                                            Some("Pending") => println!("Validate Document Extraction in progress. Waiting..."),
                                            Some("Completed") => {
                        ...
                        Some("NotStarted") | Some("Running") | Some("Unassigned") => println!("Validation request status: {}", response_data["status"]),
    ...
    pub async fn validate_classification_results(
        ...
        match Client::new().post(&api_url).headers(headers).json(&payload).send().await {
            Ok(response) => {
                if response.status().is_success() {
            ...
            match Client::new().get(&url).headers(headers).send().await {
                Ok(response) => {
                    ...
                    match response_data["status"].as_str() {
                        Some("Succeeded") => {
                            println!("Classification Validation request submitted successfully!");
                            if let Some(document_type_id) = response_data["result"]["validatedClassificationResults"][0]["DocumentTypeId"].as_str() {
