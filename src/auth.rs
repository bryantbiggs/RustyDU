...
impl Authentication {
    ...
    pub async fn get_bearer_token(&self) -> Option<String> {
        ...
        let data = [
            ...
            ("scope", &"Du.DocumentManager.Document Du.Classification.Api Du.Digitization.Api Du.Extraction.Api Du.Validation.Api".to_string()),
        ];

        return match client.post(&self.token_url)
            .form(&data)
            .send()
            .await {
            Ok(response) => {
