...
impl CSVWriter {
    ...
    pub fn write_validated_results_to_csv(
        ...
        let fields_to_extract = [
            "FieldName", "Value", "OcrConfidence", "Confidence", "IsMissing", "ActualValue", "OperatorConfirmed",
            "IsCorrect",
        ];

        writer.write_record(&fields_to_extract)?;

        if let Some(validated_fields) = validated_results["result"]["validatedExtractionResults"]["ResultsDocument"]["Fields"].as_array() {
            for validated_field in validated_fields {
                ...
                let ocr_confidence = extraction_field["Values"][0]["OcrConfidence"].as_str().unwrap_or_default();
